...
# Remove obsolete GitHub host key from the CI environment
# and install GitHub's trusted host keys.
if [ "$(id -u)" -eq 0 ]; then
    mkdir -p /root/.ssh

    ssh-keygen -f /root/.ssh/known_hosts -R github.com >/dev/null 2>&1 || true
